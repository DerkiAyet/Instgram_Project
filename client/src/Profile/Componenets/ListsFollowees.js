...
function ListFollowees({ hideList }) {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:3001/auth/user_followers_and_followees', {
      ...
      .then((res) => setListOfFollowees(res.data.followees))
  ...
  const followersFilterd = listOfFollowees.filter((follower) => {
    return (
  ...
  const followeesRef = useRef(null);

  useEffect(() => {

    const handleShowFollowers = (e) => {
      if (followeesRef.current && !followeesRef.current.contains(e.target)) {
        hideList();
      }
    }
  ...
  return (
    <div className='list-followers-container' ref={followeesRef}>
      <div className="title">
        <span>
          Following
        </span>
      ...
      <div className="followers-list">
        {
          followersFilterd.map((follower) => (
            <FolloweeAccountLine
              ...
              removeFollowerSuccess={() => setListOfFollowees(listOfFollowees.filter((f) => f.userId !== follower.userId))}
            />
          ))
        }
      </div>
...
export const FolloweeAccountLine = ({ userId, userImg, userName, fullName, removeFollowerSuccess }) => {

  const navigate = useNavigate();

  const removeFollow = () => {
    axios.delete(
      `http://localhost:3001/follow/remove_follow/${userId}`,
      {
        headers: {
          accessToken: localStorage.getItem('accessToken'),
        },
  ...
  return (
    <div className="follower-account-line">
      ...
      <button className="remove-account" onClick={() => removeFollow()}>
        Unfollow
      </button>
    </div >
  )
